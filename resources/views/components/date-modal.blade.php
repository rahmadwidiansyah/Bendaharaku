...
<button type="button" onclick="toggleDateModal()" class="bg-[#1A1A1A] border border-[#262626] text-gray-400 hover:text-white rounded-[1rem] px-4 flex items-center justify-center active:scale-95 transition-all shadow-md relative h-[48px] z-30">
    ...
    @if($startDate != \Carbon\Carbon::now()->startOfMonth()->format('Y-m-d'))
        <span class="absolute top-2 right-3 w-2 h-2 bg-[#FCA5FF] rounded-full"></span>
    @endif
</button>
...
<div id="dateModal" class="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm hidden flex flex-col justify-end transition-all duration-300 opacity-0 pointer-events-none">
    ...
    <div id="dateModalContent" class="w-full max-w-md mx-auto bg-gradient-to-b from-[#1E1E1E] to-[#121212] rounded-t-[2.5rem] border-t border-white/10 p-6 pb-12 transform translate-y-full transition-transform duration-300 relative z-[10000] pointer-events-auto shadow-[0_-10px_50px_rgba(0,0,0,0.8)]">
        ...
        <form action="{{ $action }}" method="GET" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4 text-left">
                <div class="space-y-1.5">
                    <label class="text-[10px] font-bold text-[#FCA5FF] uppercase tracking-widest pl-1">Dari</label>
                    <input type="date" name="start_date" id="modal_start_date" value="{{ $startDate }}" class="w-full bg-[#1A1A1A] border border-[#333] text-white rounded-2xl p-4 text-xs" style="color-scheme: dark;">
                </div>
                <div class="space-y-1.5">
                    <label class="text-[10px] font-bold text-[#FCA5FF] uppercase tracking-widest pl-1">Sampai</label>
                    <input type="date" name="end_date" id="modal_end_date" value="{{ $endDate }}" class="w-full bg-[#1A1A1A] border border-[#333] text-white rounded-2xl p-4 text-xs" style="color-scheme: dark;">
                </div>
            </div>
            <div class="grid grid-cols-3 gap-2">
                <button type="button" onclick="setQuickDate('7days')" class="bg-[#1A1A1A] border border-[#333] text-[9px] font-bold text-gray-400 py-3.5 rounded-xl uppercase hover:text-white transition-colors">7 Hari</button>
                <button type="button" onclick="setQuickDate('thisMonth')" class="bg-[#1A1A1A] border border-[#333] text-[9px] font-bold text-gray-400 py-3.5 rounded-xl uppercase hover:text-white transition-colors">Bulan Ini</button>
                <button type="button" onclick="setQuickDate('lastMonth')" class="bg-[#1A1A1A] border border-[#333] text-[9px] font-bold text-gray-400 py-3.5 rounded-xl uppercase hover:text-white transition-colors">Bulan Lalu</button>
            </div>
            <button type="submit" class="w-full bg-[#FCA5FF] text-[#121212] font-black text-xs uppercase py-4 rounded-2xl shadow-lg active:scale-95 transition-all">Terapkan Filter</button>
        </form>
    </div>
</div>
