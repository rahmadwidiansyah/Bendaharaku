...
<template>
    <nav :class="[
        ...
        <div :class="['px-4 mb-8 justify-end', isDesktopLayout ? 'hidden lg:flex' : 'hidden']">
            <button @click="$emit('toggle', !isSidebarOpen)"
                class="text-gray-400 hover:text-white transition-colors bg-gray-800 p-2 rounded-xl border border-white/10 hover:border-purple-500/50">
                <svg v-if="isSidebarOpen" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                    stroke-width="2">
        ...
        <div
            :class="['flex justify-around items-center pt-4 pb-1.5 px-3', isDesktopLayout ? 'lg:flex-col lg:justify-start lg:gap-4 lg:px-4 lg:pt-0' : '']">
            ...
            <div :class="['w-full mb-4', isDesktopLayout ? 'hidden lg:flex lg:flex-col lg:gap-2' : 'hidden']">
                <Link :href="route('transactions.create')"
                    ...
                    <svg class="w-6 h-6 shrink-0 transition-transform group-hover:rotate-90" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24" stroke-width="3">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                    <span
                        :class="['text-2xs font-bold tracking-wider uppercase', !isSidebarOpen ? 'lg:hidden' : '']">Catat
                        Baru</span>
                </Link>
                ...
                <a href="https://t.me/catatwidi_bot" target="_blank"
                    ...
                    <svg class="w-6 h-6 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                        <path
                            d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221l-1.97 9.28c-.145.658-.537.818-1.084.508l-3-2.21-1.446 1.394c-.14.18-.357.223-.548.223l.188-2.85 5.18-4.68c.223-.198-.054-.31-.346-.11l-6.4 4.02-2.76-.89c-.6-.188-.612-.6.126-.89l10.814-4.17c.5-.188.948.116.822.885z" />
                    </svg>
                    <span
                        :class="['text-2xs font-bold tracking-wider uppercase', !isSidebarOpen ? 'lg:hidden' : '']">Telegram</span>
                </a>
            </div>
            ...
            <Link :href="route('dashboard')"
                ...
                <svg class="w-6 h-6 shrink-0 transition-transform lg:group-hover:scale-110" fill="none"
                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
                </svg>
                <span
                    :class="['text-2xs font-bold tracking-wider uppercase', !isSidebarOpen ? 'lg:hidden' : '']">Home</span>
            </Link>
            ...
            <Link :href="route('wallets.index')"
                ...
                <span
                    :class="['text-2xs font-bold tracking-wider uppercase', !isSidebarOpen ? 'lg:hidden' : '']">Aset</span>
            </Link>
            ...
            <Link :href="route('analytics.index')"
                ...
                <span
                    :class="['text-2xs font-bold tracking-wider uppercase', !isSidebarOpen ? 'lg:hidden' : '']">Grafik</span>
            </Link>
            ...
            <Link :href="route('categories.index')"
                ...
                <span
                    :class="['text-2xs font-bold tracking-wider uppercase', !isSidebarOpen ? 'lg:hidden' : '']">Label</span>
            </Link>
