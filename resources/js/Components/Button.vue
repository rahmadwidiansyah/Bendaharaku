...
// ─── Size classes (responsive, touch-target aware) ─────────────────
// xs stays compact for inline chips; sm/md/lg enforce 44px min touch target
const sizeClasses = computed(() => ({
    xs: 'text-2xs px-2 py-1 gap-1 rounded-lg min-h-7',
    sm: 'text-2xs px-3 py-2 gap-1.5 rounded-xl min-h-9 sm:min-h-10',
    md: 'text-xs px-4 py-2.5 gap-2 rounded-xl min-h-11',
    lg: 'text-sm px-5 py-3 gap-2 rounded-xl min-h-11 sm:min-h-12',
}[props.size]))
...
const buttonClasses = computed(() => [
    ...
    'focus-visible:ring-2 focus-visible:ring-[var(--color-brand)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-surface-base)]',
    // Conditional
