...
<template>
    <div>
        <button type="button" @click="toggleModal"
            ...
            <span v-if="startDate !== new Date().toISOString().split('T')[0].substring(0, 8) + '01'"
                class="absolute top-2 right-3 w-2 h-2 bg-purple-500 rounded-full"></span>
        </button>
        ...
        <Teleport to="body">
            <div v-if="showModal"
                ...
                <div
                    class="w-full max-w-sm mx-auto bg-gradient-to-b from-gray-900 to-gray-800 rounded-2xl border border-white/10 p-6 relative z-[9999] animate-pop-in shadow-2xl">

                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-sm font-black text-white uppercase tracking-widest">Rentang Waktu</h3>
                        <button type="button" @click="toggleModal"
                            class="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-gray-400 active:scale-90 transition-all">
                    ...
                    <form @submit.prevent="submit" class="space-y-5">
                        ...
                        <div class="grid grid-cols-3 gap-2 pt-2">
                            <button type="button" @click="setQuickDate('thisYear')"
                                class="bg-gradient-to-br from-gray-900 to-gray-800 text-2xs font-bold text-gray-400 py-3 rounded-xl border border-white/10 uppercase hover:text-white transition-colors">Tahun
                                Ini</button>
                            <button type="button" @click="setQuickDate('thisMonth')"
                                class="bg-gradient-to-br from-gray-900 to-gray-800 text-2xs font-bold text-gray-400 py-3 rounded-xl border border-white/10 uppercase hover:text-white transition-colors">Bulan
                                Ini</button>
                            <button type="button" @click="setQuickDate('lastMonth')"
                                class="bg-gradient-to-br from-gray-900 to-gray-800 text-2xs font-bold text-gray-400 py-3 rounded-xl border border-white/10 uppercase hover:text-white transition-colors">Bulan
                                Lalu</button>
                        </div>
