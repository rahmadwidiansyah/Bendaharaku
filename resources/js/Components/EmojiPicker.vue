...
const selectCustom = (path) => {
    ...
    // 🔥 PERBAIKAN: path sudah ada /storage/, tidak perlu ditambahkan lagi
...
const getImageUrl = (val) => {
    ...
    // 🔥 PERBAIKAN: val sudah menyimpan path lengkap dari db/state, langsung return saja
    return val; 
};
</script>
...
<template>
    <div class="shrink-0 flex flex-col justify-end">
        ...
        <button type="button" @click="toggleModal"
            ...
            <img v-if="isImage(modelValue) || previewUrl" :src="getImageUrl(modelValue)" class="absolute inset-0 w-full h-full object-contain p-2">
            ...
            <div class="relative w-full max-w-sm bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl border border-white/10 shadow-2xl flex flex-col overflow-hidden max-h-[70vh] animate-pop-in">
                ...
                <div class="p-5 overflow-y-auto no-scrollbar flex-1 bg-gray-900">
                    ...
                    <div v-if="emojiData.find(c => c.id === activeCategory)?.type === 'custom'" class="grid grid-cols-3 gap-3">
                        ...
                        <!-- 🔥 PERBAIKAN: Looping daftar gambar bank/wallet -->
                        <button 
                            v-for="img in emojiData.find(c => c.id === activeCategory).list" 
                            :key="img" 
                            type="button" 
                            @click="selectCustom(img)" 
                            class="aspect-square bg-gray-800 border-2 border-transparent hover:border-purple-500 rounded-xl overflow-hidden p-2 transition-all active:scale-95 flex items-center justify-center hover:scale-105"
                        >
