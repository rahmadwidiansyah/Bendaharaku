...
const config = computed(() => isWarning.value
    ? { bg: 'bg-amber-500/6', border: 'border-amber-500/15', icon: '⚠️', text: 'text-amber-300', raw: 'text-amber-500/80' }
    : { bg: 'bg-red-500/6',   border: 'border-red-500/15',   icon: '❌', text: 'text-red-300',   raw: 'text-red-500/80' }
)
</script>

<template>
    <div :class="['rounded-xl border px-3.5 py-3 flex gap-2.5', config.bg, config.border]">
        <!-- Icon -->
        <span class="text-sm mt-0.5 shrink-0">{{ config.icon }}</span>
