...
<template>
    <AuthenticatedLayout :fullWidth="true">
        <Head :title="$t('ai.analyticsTitle')" />
        ...
        <div class="p-5 w-full lg:max-w-7xl mx-auto lg:px-8 relative z-10 animate-slide-up min-h-screen">
            <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pt-4 gap-4">
                ...
                <div class="flex items-center gap-3">
                    <select v-model="dateRange" @change="fetchData" class="bg-gray-900 border border-white/10 text-white rounded-xl py-2 px-4 text-xs focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400">
                        ...
                        <option :value="90">{{ $t('ai.last90Days') }}</option>
                    </select>
            ...
            <div v-else class="space-y-6">
                <div class="grid grid-cols-2 lg:grid-cols-6 gap-4">
                    <div class="bg-gray-900/50 border border-white/10 p-5 rounded-2xl flex flex-col justify-between">
                        <p class="text-2xs text-gray-400 font-bold uppercase tracking-widest mb-2">{{ $t('ai.requests') }}</p>
                    ...
                    <div class="bg-gray-900/50 border border-white/10 p-5 rounded-2xl flex flex-col justify-between relative overflow-hidden">
                        ...
                        <p class="text-2xs text-gray-400 font-bold uppercase tracking-widest mb-2">{{ $t('ai.success') }}</p>
                        <h3 class="text-2xl font-black text-emerald-400">{{ overview.success_rate }}%</h3>
                    </div>
                    <div class="bg-gray-900/50 border border-white/10 p-5 rounded-2xl flex flex-col justify-between">
                        <p class="text-2xs text-gray-400 font-bold uppercase tracking-widest mb-2">{{ $t('ai.drafts') }}</p>
                    ...
                    <div class="bg-gray-900/50 border border-white/10 p-5 rounded-2xl flex flex-col justify-between relative overflow-hidden">
                        ...
                        <p class="text-2xs text-gray-400 font-bold uppercase tracking-widest mb-2">{{ $t('ai.correctionRate') }}</p>
                        <h3 class="text-2xl font-black text-rose-400">{{ overview.correction_rate }}%</h3>
                    </div>
                    <div class="bg-gray-900/50 border border-white/10 p-5 rounded-2xl flex flex-col justify-between">
                        <p class="text-2xs text-gray-400 font-bold uppercase tracking-widest mb-2">{{ $t('ai.tokens') }}</p>
                        <h3 class="text-2xl font-mono text-indigo-300">{{ formatNumber(overview.total_tokens) }}</h3>
                    </div>
                    <div class="bg-gray-900/50 border border-white/10 p-5 rounded-2xl flex flex-col justify-between">
                        <p class="text-2xs text-gray-400 font-bold uppercase tracking-widest mb-2">{{ $t('ai.estCost') }}</p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="bg-gray-900/50 border border-white/10 p-6 rounded-2xl lg:col-span-1 h-[400px] flex flex-col">
                        <h3 class="text-sm font-bold text-white mb-4">{{ $t('ai.charts.trafficByProvider') }}</h3>
                    ...
                    <div class="bg-gray-900/50 border border-white/10 p-6 rounded-2xl lg:col-span-2 h-[400px] flex flex-col">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <h3 class="text-sm font-bold text-white">{{ $t('ai.charts.confidenceTrend') }}</h3>
                                <p class="text-2xs text-gray-400 mt-1">{{ $t('ai.charts.confidenceTrendDesc') }}</p>
                            </div>
                        </div>
                        <div class="relative flex-1 w-full">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-3" v-if="learning.top_memories?.length">
                            <div v-for="mem in learning.top_memories" :key="mem.keyword_pattern" class="flex justify-between items-center p-3 bg-gray-800/50 rounded-lg border border-white/5">
                                ...
                                <div class="flex gap-4 text-2xs font-mono text-gray-400">
                                    <span>{{ $t('ai.hits') }}: <span class="text-white">{{ mem.hit_count }}</span></span>
                                    <span>{{ $t('ai.weight') }}: <span class="text-indigo-400">{{ parseFloat(mem.effective_weight).toFixed(2) }}</span></span>
                                </div>
                            </div>
                        </div>
                        <div v-else class="text-sm text-gray-500 py-4 text-center">{{ $t('ai.emptyMemory') }}</div>
                    </div>
                    ...
                    <div class="bg-gray-900/50 border border-white/10 p-6 rounded-2xl">
                        <h3 class="text-sm font-bold text-white mb-4 flex items-center gap-2">
                            ...
                            {{ $t('ai.charts.correctedCategories') }}
                        </h3>
                        <div class="space-y-3" v-if="learning.top_corrected_categories?.length">
                            <div v-for="cat in learning.top_corrected_categories" :key="cat.category_id" class="flex justify-between items-center p-3 bg-rose-500/10 rounded-lg border border-rose-500/10">
                                <span class="text-sm font-medium text-rose-300">{{ $t('ai.categoryId') }}: {{ cat.category_id }}</span>
                                <span class="text-2xs font-bold px-2 py-1 bg-rose-500/20 text-rose-300 rounded">{{ cat.count }} {{ $t('ai.corrections') }}</span>
                            </div>
                        </div>
                        <div v-else class="text-sm text-gray-500 py-4 text-center">{{ $t('ai.emptyCorrections') }}</div>
                    </div>
                </div>
