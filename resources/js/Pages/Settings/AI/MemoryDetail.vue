...
const actionBadge = (action: string) => {
  const badges: Record<string, string> = {
    ...
    UPDATED: 'bg-purple-600/20 text-purple-400 border-purple-600/30',
  ...
  return badges[action] || 'bg-gray-600/20 text-gray-400 border-gray-600/30';
};
</script>
    ...
    <SettingsLayout>
      <template #header>
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            <h2 class="text-2xl font-black text-white tracking-tight leading-none">{{ t('settings.ai.memory.detail.title') }}</h2>
            <p class="text-sm text-gray-400 mt-1.5 font-medium truncate">{{ memory.keyword }}</p>
          </div>
        </div>
      </template>
      ...
      <div class="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-4 sm:mb-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
          <div>
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.keyword') }}</span>
            <span class="text-white font-medium">{{ memory.keyword }}</span>
          </div>
          <div v-if="memory.raw_subject">
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.raw_subject') }}</span>
            <span class="text-white">{{ memory.raw_subject }}</span>
          </div>
          <div v-if="memory.normalized_subject">
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.normalized_subject') }}</span>
            <span class="text-white">{{ memory.normalized_subject }}</span>
          </div>
          <div v-if="memory.category">
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.category') }}</span>
            <span class="text-white">{{ memory.category }}</span>
          </div>
          <div v-if="memory.wallet">
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.wallet') }}</span>
            <span class="text-white">{{ memory.wallet }}</span>
          </div>
          <div>
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.current_weight') }}</span>
            <span :class="['inline-block px-2 py-0.5 rounded text-xs font-medium mt-0.5', weightColor(memory.weight)]">
              {{ memory.weight.toFixed(2) }}
            </span>
          </div>
          <div>
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.hit_count') }}</span>
            <span class="text-white">{{ memory.hit_count }}×</span>
          </div>
          <div>
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.created_at') }}</span>
            <span class="text-white">{{ memory.created_at }}</span>
          </div>
          <div v-if="memory.last_applied_at">
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.last_used') }}</span>
            <span class="text-white">{{ memory.last_applied_at }}</span>
          </div>
          <div>
            <span class="text-gray-500 text-xs block">{{ t('settings.ai.memory.detail.algorithm_version') }}</span>
            <span class="text-white text-xs bg-gray-800 px-2 py-0.5 rounded">{{ memory.algorithm_version }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
        ...
        <div v-if="logs && logs.length > 0" class="relative">
          <div class="absolute left-[17px] top-2 bottom-2 w-0.5 bg-gray-800" />

          <div v-for="(log, i) in logs" :key="log.id" class="relative flex gap-4 pb-6 last:pb-0">
            <div class="shrink-0 relative z-10">
              <div class="w-[34px] h-[34px] rounded-full bg-gray-800 border-2 border-gray-900 flex items-center justify-center">
            ...
            <div class="flex-1 min-w-0 pt-1">
              ...
              <div v-if="log.raw_subject" class="text-xs text-gray-400 mt-1.5">
                {{ log.raw_subject }}
              </div>

              <div v-if="(log.old_weight !== null && log.new_weight !== null) || (log.old_hit_count !== null && log.new_hit_count !== null)" class="flex gap-3 mt-1.5 text-xs text-gray-500">
              ...
              <div v-if="log.reason" class="text-xs text-gray-500 mt-1 italic">
                {{ log.reason }}
              </div>
        ...
        <div v-else class="text-center py-8">
          <p class="text-sm text-gray-500">{{ t('settings.ai.memory.detail.timeline_empty') }}</p>
        </div>
      </div>
    </SettingsLayout>
  </AuthenticatedLayout>
