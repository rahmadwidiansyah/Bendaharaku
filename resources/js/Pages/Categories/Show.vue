    ...
    <AuthenticatedLayout>
        ...
        <div class="p-5 pb-32 max-w-md mx-auto relative animate-fade-in-up">
            ...
            <header class="flex justify-between items-center mb-8 pt-4 relative z-10">
                ...
                <Link v-if="!isSystem" :href="route('categories.edit', category.id)" class="w-10 h-10 rounded-full bg-[#1A1A1A] border border-[#262626] flex items-center justify-center text-purple-400 active:scale-95 transition-all">
            ...
            <div class="flex flex-col items-center mb-10 relative z-10">
                <div class="w-24 h-24 rounded-xl bg-[#1A1A1A] border border-[#262626] flex items-center justify-center text-5xl shadow-2xl mb-5 overflow-hidden relative p-1">
                    <img v-if="category.icon?.includes('.')" :src="'/storage/' + category.icon" class="w-full h-full object-cover rounded-xl">
                ...
                <div class="text-center">
                    ...
                    <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#1A1A1A] border border-[#262626]">
                        <span class="w-1.5 h-1.5 rounded-full" :class="category.type.name === 'Income' ? 'bg-green-400 shadow-[0_0_8px_#4ade80]' : 'bg-gray-500'"></span>
            ...
            <div class="space-y-4 relative z-10">
                <div class="flex items-center justify-between px-1 mb-2">
                    ...
                    <span class="text-xs font-bold text-gray-600 bg-[#1A1A1A] px-2 py-0.5 rounded-xl border border-[#262626]">{{ transactions.length }} Record</span>
                </div>
                ...
                <button v-for="trx in transactions" :key="trx.id" type="button" @click="openDetail(trx)"
                    ...
                    <div class="flex-1 min-w-0 pr-3 relative z-10">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <p class="text-[9px] text-gray-600 font-bold uppercase">{{ new Date(trx.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) }}</p>
                        </div>
                        
                        <div class="mb-2">
                            <p class="text-sm font-bold text-white truncate">{{ trx.notes ?? 'Tanpa catatan' }}</p>
                            <p v-if="trx.subject && trx.subject !== '-'" class="text-xs text-[#E5D07E] font-bold mt-0.5 flex items-center gap-1">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="flex items-center gap-1.5 bg-[#262626] px-2 py-1 rounded-xl border border-[#333]">
                                <span class="text-[8px] font-bold text-gray-400 uppercase truncate max-w-[60px]">{{ trx.source_wallet?.name }}</span>
                                <svg class="w-2.5 h-2.5 text-purple-400/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                                <span class="text-[8px] font-bold text-white uppercase truncate max-w-[60px]">{{ trx.destination_wallet?.name }}</span>
                            </div>
                        </div>
                    </div>

                    <div class="text-right shrink-0 relative z-10">
                        <p class="text-base font-black" :class="category.type.name === 'Income' ? 'text-green-400' : 'text-white'">
                            {{ category.type.name === 'Income' ? '+' : '-' }}{{ formatNumber(trx.amount) }}
                        </p>
                        <span class="inline-block text-[7px] uppercase tracking-widest font-black px-1.5 py-0.5 rounded border" :class="getTypeColor(trx.type.name)">
                            {{ trx.type.name }}
                        </span>
                    </div>
                </button>

                <div v-if="transactions.length === 0" class="text-center py-16 bg-[#1A1A1A] rounded-xl border-2 border-dashed border-[#262626]">
