        ...
        <div class="p-5 pb-32 w-full lg:max-w-4xl mx-auto lg:px-8 relative animate-fade-in-up">
            ...
            <header class="flex justify-between items-center mb-8 pt-4 relative z-10">
                <button type="button" @click="handleBack"
                    class="w-10 h-10 rounded-full bg-gradient-to-br from-gray-900 to-gray-800 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white active:scale-95 transition-all shadow-md">
                ...
                <Link v-if="!isSystem" :href="route('categories.edit', category.id)"
                    class="w-10 h-10 rounded-full bg-gradient-to-br from-gray-900 to-gray-800 border border-white/10 flex items-center justify-center text-purple-500 active:scale-95 transition-all">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div class="flex flex-col items-center mb-10 relative z-10">
                <div
                    class="w-24 h-24 rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-white/10 flex items-center justify-center text-5xl shadow-2xl mb-5 overflow-hidden relative p-1">
                ...
                <div class="text-center">
                    ...
                    <div
                        class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-br from-gray-900 to-gray-800 border border-white/10">
                        <span class="w-1.5 h-1.5 rounded-full"
                            :class="category.type.name === 'Income' ? 'bg-green-400' : 'bg-gray-500'"></span>
                        <p class="text-xs font-bold text-gray-400 uppercase tracking-[0.2em]">{{ category.type.name ===
            ...
            <div
                class="bg-gradient-to-br from-gray-900 to-gray-800 border border-white/10 rounded-xl p-7 text-center mb-10 shadow-2xl relative overflow-hidden group z-10">
                <p class="text-xs font-bold text-gray-500 uppercase tracking-[0.2em] mb-2 opacity-60">Total Akumulasi
                </p>
            ...
            <div class="space-y-4 relative z-10">
                <div class="flex items-center justify-between px-1 mb-2">
                    ...
                    <span
                        class="text-xs font-bold text-gray-500 bg-gradient-to-br from-gray-900 to-gray-800 border-white/10 px-2 py-0.5 rounded-xl border">{{
                        transactions.length }} Record</span>
                </div>

                <button v-for="trx in transactions" :key="trx.id" type="button" @click="openDetail(trx)"
                    class="w-full text-left bg-gradient-to-br from-gray-900 to-gray-800 p-4 rounded-xl border border-white/10 flex justify-between items-center active:scale-[0.98] transition-all shadow-sm relative overflow-hidden group">

                    <div class="flex-1 min-w-0 pr-3 relative z-10">
                        <div class="flex items-center gap-2 mb-2">
                            <p class="text-xs font-black text-gray-500 uppercase tracking-tighter italic">
                                {{ new Date(trx.date).toLocaleDateString('id-ID', {
                                    day: '2-digit', month: 'short',
                                year: 'numeric' }) }}
                            </p>
                            <span class="w-1 h-1 bg-gray-700 rounded-full"></span>
                            <p class="text-xs text-gray-600 font-bold uppercase">{{ new
                        ...
                        <div class="mb-2">
                            ...
                            <p v-if="trx.subject && trx.subject !== '-'"
                                class="text-xs text-yellow-500 font-bold mt-0.5 flex items-center gap-1">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-xs font-bold text-gray-400 uppercase truncate max-w-[60px]">{{
                                trx.source_wallet?.name }}</span>
                            <svg class="w-2.5 h-2.5 text-purple-400/50" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="3">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                            </svg>
                            <span class="text-xs font-bold text-white uppercase truncate max-w-[60px]">{{
                    ...
                    <div class="text-right shrink-0 relative z-10">
                        ...
                        <span
                            class="inline-block text-xs uppercase tracking-widest font-black px-1.5 py-0.5 rounded border"
                ...
                <div v-if="transactions.length === 0"
                    class="text-center py-16 bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl border-2 border-dashed border-white/10">
