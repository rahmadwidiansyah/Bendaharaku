    ...
    <AuthenticatedLayout :fullWidth="true" :hideNav="true">
            ...
            <div class="shrink-0 border-b border-white/5">
                ...
                <div class="flex items-center justify-between px-4 pt-3 pb-2">
                    ...
                    <div class="flex items-center gap-2">
                        <button type="button" @click="destroy" class="w-8 h-8 flex items-center justify-center rounded-full bg-red-500/10 border border-red-500/20 text-[var(--color-expense-text)] hover:bg-red-500/20 active:scale-90 transition-all" :aria-label="$t('transaction.deleteTitle')">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                        </button>
                        <button type="button" @click="goBack" class="w-8 h-8 flex items-center justify-center rounded-full bg-gray-800 border border-white/10 text-gray-400 hover:text-[var(--color-expense-text)] hover:border-red-500/30 active:scale-90 transition-all" :aria-label="$t('common.close')">
            ...
            <Transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 translate-y-8" enter-to-class="opacity-100 translate-y-0">
                <form v-if="formStep >= 3 && mainTab !== 'Transfer'" @submit.prevent="submit" class="flex-1 flex flex-col min-h-0 overflow-hidden border-t border-white/5">
                    ...
                    <div v-if="Object.keys(form.errors).length" class="shrink-0 mx-4 mt-3 p-3 bg-red-500/10 border border-red-500/30 rounded-xl">
                        <p v-for="(err, key) in form.errors" :key="key" class="text-[var(--color-expense-text)] text-2xs font-bold">{{ err }}</p>
                    </div>
                    ...
                    <div class="shrink-0 px-4 pb-2 pt-1">
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="submit" :disabled="form.processing" class="flex-1 h-12 bg-[var(--color-income-text)] hover:bg-green-500 rounded-xl flex items-center justify-center gap-2 text-white font-black text-sm uppercase tracking-wider active:scale-95 transition-all shadow-lg shadow-green-600/30 disabled:opacity-50">
        ...
        <BaseModal :show="showDeleteConfirm" max-width="sm" @close="showDeleteConfirm = false">
            <div class="text-center px-1">
                <div class="w-14 h-14 rounded-full bg-red-500/15 text-[var(--color-expense-text)] mx-auto flex items-center justify-center mb-4 border border-red-500/20">
