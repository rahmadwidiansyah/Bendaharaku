        ...
        <div class="fixed inset-0 z-60 flex flex-col bg-gray-900 text-white" :class="isDesktopLayout ? 'lg:relative lg:inset-auto lg:z-0' : ''" style="height: 100dvh;">
                ...
                <div class="flex items-center gap-2 px-4 pb-2.5 overflow-x-auto no-scrollbar">
                    ...
                    <button v-else type="button" @click="resetToStep(2)"
                        ...
                        <span v-if="formStep > 2 && selectedCategory">
                            <AppIcon :icon="selectedCategory.icon" class="inline w-3.5 h-3.5 text-purple-400" />
                    ...
                    <template v-if="mainTab !== 'Transfer'">
                        ...
                        <div v-if="formStep >= 3" class="px-3 py-1.5 rounded-full text-2xs font-black uppercase tracking-widest bg-purple-500/20 text-purple-300 border border-purple-500/40 shrink-0">3 · {{ $t('transaction.amount') }}</div>
                    </template>
                </div>
            </div>
            ...
            <Transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 translate-y-4" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition-all duration-200 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-4">
                <div v-if="formStep === 2" class="flex-1 flex flex-col min-h-0 overflow-hidden">
                    ...
                    <input type="text" v-model="form.subject" :placeholder="$t('transaction.namePlaceholder')" @input="form.subject = $event.target.value.toUpperCase()" class="w-full bg-gray-800 border border-white/10 focus:border-purple-500 rounded-xl px-4 py-3 text-center text-lg font-bold text-white focus:ring-0 placeholder-gray-700 outline-none uppercase" />
                        ...
                        <div v-if="(activeType === 'Debt' && debtSubTab === 'income') || (activeType === 'Receivable' && debtSubTab === 'expense')" class="p-4 bg-gray-800/60 rounded-xl border border-white/5 space-y-3">
                            ...
                            <template v-if="form.due_date_type !== null">
                                <div class="flex gap-2">
                                    <button type="button" @click="form.due_date_type = 'fixed'" class="flex-1 py-2 text-2xs font-bold uppercase rounded-lg transition-all" :class="form.due_date_type === 'fixed' ? 'bg-purple-600 text-white' : 'bg-gray-900 text-gray-500'">{{ $t('transaction.fixedDate') }}</button>
                                    <button type="button" @click="form.due_date_type = 'monthly'" class="flex-1 py-2 text-2xs font-bold uppercase rounded-lg transition-all" :class="form.due_date_type === 'monthly' ? 'bg-purple-600 text-white' : 'bg-gray-900 text-gray-500'">{{ $t('transaction.everyMonth') }}</button>
                                    <button type="button" @click="form.due_date_type = 'daily'" class="flex-1 py-2 text-2xs font-bold uppercase rounded-lg transition-all" :class="form.due_date_type === 'daily' ? 'bg-purple-600 text-white' : 'bg-gray-900 text-gray-500'">{{ $t('transaction.everyDay') }}</button>
                                </div>
                                ...
                                <input v-if="form.due_date_type === 'monthly'" type="number" min="1" max="31" v-model="form.due_date_interval" :placeholder="$t('transaction.dayPlaceholder')" class="w-full bg-gray-900 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:ring-0 focus:border-purple-500 text-center" />
                                <input v-if="form.due_date_type === 'daily'" type="number" min="1" v-model="form.due_date_interval" :placeholder="$t('transaction.cyclePlaceholder')" class="w-full bg-gray-900 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:ring-0 focus:border-purple-500 text-center" />
                            </template>
                        </div>
                        <button type="button" @click="goToNominal" class="w-full py-4 rounded-2xl bg-purple-600 text-white text-sm font-black uppercase tracking-widest transition-all active:scale-95">{{ $t('transaction.nextNominal') }}</button>
                    </div>
                        ...
                        <div class="flex-1 overflow-y-auto no-scrollbar px-4 pb-4">
                            <div v-if="activeCategories.length" class="grid grid-cols-3 gap-2.5">
                                <button v-for="cat in activeCategories" :key="cat.id" type="button" @click="selectCategory(cat); goToNominal()"
                                    ...
                                    <AppIcon :icon="cat.icon" fallback="folder" class="w-8 h-8 mb-2 text-purple-400" />
